...
public class CategoryApiController implements CrudInterface<CategoryApiRequest, CategoryApiResponse> {
    ...
    @Override
    @PutMapping("/{id}")
    public Header<CategoryApiResponse> update(@PathVariable Long id,
                                              @RequestBody Header<CategoryApiRequest> request) {
        return categoryApiLogicService.update(id, request);
    }
